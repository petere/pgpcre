...
pcre_in(PG_FUNCTION_ARGS)
{
	...
	if (GetDatabaseEncoding() == PG_UTF8)
		pc = pcre2_compile((unsigned char *) input_string, in_strlen, PCRE2_UTF | PCRE2_UCP, &err, &erroffset, NULL);
	else if (GetDatabaseEncoding() == PG_SQL_ASCII)
		pc = pcre2_compile((unsigned char *) input_string, in_strlen, 0, &err, &erroffset, NULL);
	else
	{
		char *utf8string;
		...
		utf8string = (char *) pg_do_encoding_conversion((unsigned char *) input_string,
														...
														GetDatabaseEncoding(),
														PG_UTF8);
		pc = pcre2_compile((unsigned char *) utf8string, strlen(utf8string), PCRE2_UTF | PCRE2_UCP, &err, &erroffset, NULL);
		if (utf8string != input_string)
			pfree(utf8string);
	}
	if (!pc)
		elog(ERROR, "PCRE compile error: %d", err); // TODO: pcre2_get_error_message
...
matches_internal(text *subject, pgpcre *pattern, char ***return_matches, int *num_captured)
{
	...
	if (num_captured)
	{
		...
		if ((rc = pcre2_pattern_info(pc, PCRE2_INFO_CAPTURECOUNT, &num_substrings)) != 0)
			elog(ERROR, "pcre_pattern_info error: %d", rc);
	}
	...
	if (GetDatabaseEncoding() == PG_UTF8 || GetDatabaseEncoding() == PG_SQL_ASCII)
	{
		...
		rc = pcre2_match(pc, (unsigned char *) VARDATA_ANY(subject), VARSIZE_ANY_EXHDR(subject), 0, 0, md, NULL);
	}
	else
	{
		utf8string = (char *) pg_do_encoding_conversion((unsigned char *) VARDATA_ANY(subject),
														...
														GetDatabaseEncoding(),
														PG_UTF8);
		rc = pcre2_match(pc, (unsigned char *) utf8string, strlen(utf8string), 0, 0, md, NULL);
	}
	...
	if (return_matches)
	{
		...
		if (num_captured)
		{
			...
			*num_captured = num_substrings;
			...
			for (i = 1; i <= num_substrings; i++)
			{
				...
				{
					matches[i - 1] = palloc(ovector[i * 2 + 1] - ovector[i * 2] + 1);
					strlcpy(matches[i - 1], utf8string + ovector[i * 2], ovector[i * 2 + 1] - ovector[i * 2] + 1);
				}
			}
		}
		else
		{
			...
			matches[0] = palloc(ovector[1] - ovector[0] + 1);
			strlcpy(matches[0], utf8string + ovector[0], ovector[1] - ovector[0] + 1);
		}
